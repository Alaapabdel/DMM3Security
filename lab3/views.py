...
def register(request):
    if request.method == 'POST':
        try:
            if request.POST.get("user") == "" or request.POST.get("pass") == "":
                args = {}
                args['message'] = "Please fill in your data!"
                return render(request, 'register.html',args)
            User.objects.create(username = request.POST.get("user"), password = request.POST.get("pass"))
        except:
            args = {}
            args['message'] = "Try another Username!"
            return render(request, 'register.html',args)
        return redirect('lab3:login')
